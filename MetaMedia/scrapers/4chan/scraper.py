...
class fourchanScraper():
    ...
    def scrapeBoard(self, board): 
        #board = board

        print ("The current working directory is " + self.saveDirectory)  

        #First we'll try to create a folder for the board.
        today = datetime.date.today()
        path = self.saveDirectory  + board 
        ...
        allThreadFile = self.saveDirectory + board + "/frontPage-" + str(datetime.datetime.now()) + ".csv"
        ...
        while i < 20:
            ...
            path = self.saveDirectory + board + "/" + str(frontPage[i]['no']) + " - " + frontPage[i]['semantic_url']
            ...
            url = "https://a.4cdn.org/" + board + "/thread/" + str(frontPage[i]['no']) + ".json"
            response = urllib.urlopen(url)
            ...
            for post in individualThread['posts']:
                ...
                if 'tim' in individualThread['posts'][j]:
                    ...
                    try:
                        postFile.retrieve("https://i.4cdn.org/" + board + "/" + renamedFile + fileExtension, saveDirectory + "/" + board + "/" + str(frontPage[i]['no']) + " - " + frontPage[i]['semantic_url'] + "/" + filename)
